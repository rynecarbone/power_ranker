...
TODO: vectorize code to work with v3 schedule table
...
class LSQ(object):
  ...
  def _rank_pass(self, teams, passN=0, prev_rank=None):
    ...
    :param teams: data for all teams in league
    :param passN: which iteration
    :param prev_rank: list of previous iteration ranks
      (supplied after pass 1)
    :return: rankings
    """
    ...
    if passN == 0:
      # equal weight first iteration
      sig_g = np.ones(len(N_g))
    else:
  ...
  def get_ranks(self, teams):
    ...
    # List of rankings for each pass
    # Pass 0 has weight = 1
    rank_p = [self._rank_pass(teams, passN=0)]
